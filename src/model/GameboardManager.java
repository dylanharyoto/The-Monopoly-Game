...
public class GameboardManager {
    ...
    public static boolean designMap (String filepath) {
            ...
            for (String squareObjStr : squareObjects) {
                ...
                switch (type) {
                    case "P":
                        ...
                        int price = Integer.parseInt(detailsStr.split("\"price\":")[1].split(",")[0]);
                        int rent = Integer.parseInt(detailsStr.split("\"rent\":")[1].split(",")[0]);
                        properties.add(new Property(squareId, propertyName, price, rent));
                        break;
                    case "G":
                        functionalSquares.add(new Go(squareId));
                        break;
                    case "C":
                        functionalSquares.add(new Chance(squareId));
                        break;
                    case "I":
                        functionalSquares.add(new IncomeTax(squareId));
                        break;
                    case "F":
                        functionalSquares.add(new FreeParking(squareId));
                        break;
                    case "J":
                        functionalSquares.add(new GoJail(squareId));
                        break;
                    case "V":
                        functionalSquares.add(new InJailOrJustVisiting(squareId));
                        break;
                }
        ...
        GameView.displayAllProperties(properties);
        while(true) {
            int choice = Integer.parseInt(GameView.inputPrompt("""
                    ...
                    , new String[]{"1", "2", "3"}));
            if(choice == 1) {
                int propertyIndex = Integer.parseInt(GameView.inputPrompt("""
                ...
                GameView.displayProperty(updatingProperty);

                int attributeOption = Integer.parseInt(GameView.inputPrompt("""
                ...
                switch (attributeOption) {
                    case 1:
                        String newName = GameView.updateName("Please input the new name of the property, the new name should only contain 0-9 and English letters. The new name is ");
                        updatingProperty.setName(newName);
                        System.out.println("Successfully update the Name of the property!");
                        break;
                    case 2:
                        int newPrice = GameView.updateInteger("Please input the new price of the property, the new name should be a positive integer. The new price is ");
                        updatingProperty.setPrice(newPrice);
                        System.out.println("Successfully update the Price of the property!");
                        break;

                    case 3:
                        int newRent = GameView.updateInteger("Please input the new rent of the property, the new rent should be a positive integer. The new rent is ");
